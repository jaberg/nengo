...
class DotIncShapeError(Exception):
    def __init__(self, A, X, Y, tag):
        self.A = A
        self.X = X
        self.Y = Y
        self.tag = tag
    ...
    def __str__(self):
        return 'DotIncShapeError in "%s": %s x %s -> %s' % (
            self.tag,
            self.A.shape, self.X.shape, self.Y.shape)
...
class DotInc(Operator):
    ...
    def __init__(self, A, X, Y, tag=None):
        ...
        self.Y = Y
        self.tag = tag
        if len(A.shape) == 0:  # -- scaling of X
            if X.shape != Y.shape:
                raise DotIncShapeError(A, X, Y, tag)
        elif len(A.shape) == 1:
            raise DotIncShapeError(A, X, Y, tag)
        else:
            if X.ndim != Y.ndim:
                raise DotIncShapeError(A, X, Y, tag)
            if X.ndim == 1:  # matrix-vector case
                if (A.shape[1] != X.shape[0] 
                    or A.shape[0] != Y.shape[0]):
                    raise DotIncShapeError(A, X, Y, tag)
            if X.ndim == 2:  # matrix-matrix case
                if (A.shape[1] != X.shape[0] 
                    or A.shape[0] != Y.shape[0]
...
class SimLIF(Operator):
    ...
    def make_step(self, dct, dt):
        ...
        def step():
            nonlinearities.lif_step(dt, J, v, rt, output,
                                    self.nl.tau_rc,
                                    self.nl.tau_ref)
        return step
